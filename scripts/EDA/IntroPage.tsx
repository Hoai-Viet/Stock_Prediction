...
const MIN_OVERVIEW_CONFIDENCE = 0.65;
...
const OVERVIEW_TEXT = {
    en: {
        ...
        topWinnersTitle: "Top high-win symbols",
        noTopWinners: "No confirmed winners yet.",
        winningReturnTitle: "Return from correct predictions",
        noteTitle: "Note",
    ...
    vi: {
        ...
        noCandidates: (date: string) => `Không có mã BUY hoặc SELL cho ngày ${date}.`,
        ...
        candidatesDescription: (
            <>
                Dùng tín hiệu <span className="font-semibold text-buy">BUY</span> và{" "}
                <span className="font-semibold text-sell">SELL</span> sau khi giá đóng cửa đã được xác nhận.
        ...
        emptyCandidates: "Không có mã BUY hoặc SELL cho phiên này.",
        historyTitle: "Lịch sử kiểm chứng",
        historyDescription: "Các dự báo BUY/SELL đã có kết quả trong 2 tháng gần nhất, chỉ dựa trên dữ liệu lịch sử.",
        ...
        buyCandidates: "ứng viên BUY",
        sellCandidates: "ứng viên SELL",
        holdSymbols: "mã HOLD không hiển thị trong bảng",
        topWinnersTitle: "Top mã có tỷ lệ thắng cao",
        noTopWinners: "Chưa có mã thắng đã xác nhận.",
        winningReturnTitle: "Tỷ suất sinh lời từ dự báo đúng",
        noteTitle: "Lưu ý",
...
export default function IntroPage() {
    ...
    const summary = useMemo(() => {
        const filteredSignals = signals.filter(
            (item) => item.predicted_label !== "HOLD" && item.confidence != null && Number(item.confidence) >= MIN_OVERVIEW_CONFIDENCE
        );
    ...
    const topWinningSymbols = useMemo(() => {
        const stats = new Map<string, { correct: number; total: number }>();

        historicalPredictions.forEach((item) => {
            const current = stats.get(item.symbol_code) ?? { correct: 0, total: 0 };
            current.total += 1;
            if (item.is_correct) current.correct += 1;
            stats.set(item.symbol_code, current);
        });
        ...
        return Array.from(stats.entries())
            .map(([symbol, stat]) => ({
                ...
                total: stat.total,
                winRate: stat.total === 0 ? 0 : stat.correct / stat.total,
            }))
            .filter((item) => item.total >= 1 && item.correct > 0)
            .sort((a, b) => b.winRate - a.winRate || b.correct - a.correct || a.symbol.localeCompare(b.symbol))
            .slice(0, 5);
    }, [historicalPredictions]);

    const winningReturn = useMemo(() => {
        const returns = historicalPredictions
            .filter((item) => item.is_correct && item.return_pct != null)
            .map((item) => Number(item.return_pct));
    ...
    const predictionBreakdown = useMemo(() => {
        ...
        const parts = [];
        if (summary.buy > 0) parts.push(`${summary.buy} BUY`);
        if (summary.sell > 0) parts.push(`${summary.sell} SELL`);
    ...
    }, [loading, summary.buy, summary.sell, summary.tradeDate, text]);
    ...
    const predictionFilterLabel = predictionFilter === "ALL" ? text.filters.all : predictionFilter;
    const allDatesLabel = language === "vi" ? "Tất cả ngày" : "All dates";
    const historyPredictionFilterLabel = historyPredictionFilter === "ALL" ? text.filters.all : historyPredictionFilter;
    ...
    const secondaryNews = externalNews.slice(1, 3);
        ...
        <div className="animate-[fadeIn_0.45s_ease-out]">
            ...
            <main className="mx-auto grid max-w-7xl gap-6 px-4 py-6 sm:px-6 lg:grid-cols-[minmax(0,1fr)_340px] lg:px-8">
                                    ...
                                    {isPredictionFilterOpen && (
                                        <div className="absolute right-0 top-[calc(100%+6px)] z-30 w-full overflow-hidden rounded-lg border border-border-subtle bg-white py-1 shadow-[0_16px_34px_rgba(21,33,47,0.14)]">
                                            {predictionFilterOptions.map((option) => {
                                                const label = option === "ALL" ? text.filters.all : option;
                                            ...
                                            >
                                                ...
                                                <td className="px-4 py-4 text-center">
                                                    {item.is_correct == null ? (
                                                        <span className="inline-flex min-w-[54px] items-center justify-center rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-text-muted">
                                                            -
                                                        </span>
                                                    ) : item.is_correct ? (
                ...
                <section
                    ...
                    className="scroll-mt-28 overflow-hidden rounded border border-border-subtle bg-white shadow-[0_10px_30px_rgba(21,33,47,0.08)]"
                >
                ...
                <aside className="space-y-6">
                    <div className="rounded border border-border-subtle bg-white p-5 shadow-[0_10px_30px_rgba(21,33,47,0.08)]">
                        ...
                        <div className="mt-5 border-t border-border-subtle pt-4">
                            <h3 className="text-sm font-semibold text-text-primary">{text.topWinnersTitle}</h3>
                            <div className="mt-3 space-y-2">
                                {topWinningSymbols.length === 0 ? (
                                    <p className="text-sm text-text-secondary">{text.noTopWinners}</p>
                                ) : (
                                    topWinningSymbols.map((item) => (
                                        <div key={item.symbol} className="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2">
                                            <span className="text-sm font-semibold text-text-primary">{item.symbol}</span>
                                            <span className="text-xs font-bold text-buy">
                                                {formatConfidence(item.winRate)}
                                            </span>
                                        </div>
                                    ))
                                )}
                        ...
                        <div className="mt-5 rounded-lg border border-buy/15 bg-buy/5 px-3 py-3">
                            <p className="text-xs font-semibold uppercase tracking-[0.12em] text-text-secondary">
                                {text.winningReturnTitle}
                            </p>
                            <p className={`mt-1 text-2xl font-black ${winningReturn != null && winningReturn < 0 ? "text-sell" : "text-buy"}`}>
                                {formatPercentValue(winningReturn)}
                            </p>
                        </div>
                    </div>

                    <div className="rounded-2xl border border-buy/20 bg-[linear-gradient(135deg,rgba(255,255,255,0.96)_0%,rgba(235,245,241,0.96)_100%)] p-4 pb-[41px] text-text-secondary shadow-[0_16px_38px_rgba(15,23,42,0.08)]">
                        ...
                        <a
                            href="/news"
                            className="mt-4 inline-flex items-center gap-2 rounded-xl bg-buy px-4 py-2.5 text-sm font-semibold text-white transition-transform hover:-translate-y-0.5 hover:bg-buy-hover"
                        >
                            {newsButton}
