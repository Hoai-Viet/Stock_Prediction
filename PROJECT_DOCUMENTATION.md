...
flowchart TD
    ...
    subgraph DWH["🏢 Data Warehouse"]
        G1[fact_decision<br/>ML prediction + final decision]
...
#### `predict.py` — Dự đoán hàng ngày

| Mục | Chi tiết |
|-----|---------|
| **Chức năng** | Load model đã train, dự đoán BUY/SELL/SILENT cho tất cả mã cổ phiếu |
| **Input** | Daily metrics (30 ngày lookback) + Quarterly metrics (latest as-of) từ `dwh.fact_metric` |
| **Output** | `dwh.fact_decision` (predicted_label, model_version, entry_window, entry_time) |
| **Entry Timing** | ML-based (nếu có model) hoặc rule-based fallback (chọn giá thấp nhất cho BUY, cao nhất cho SELL) |
| **Position Tracking** | Tự động mở/đóng vị thế trong `dwh.fact_position` (holding_days = 3 trading days) |
| **Expected Exit** | Tính bằng **trading calendar** thực tế (bỏ qua weekend/holiday) |
| **Schedule** | Hàng ngày 15:15 ICT, step 3 trong `stock_daily_pipeline` DAG |
...
| **Output** | Hiển thị và ghi kết quả cuối cùng trực tiếp vào `dwh.fact_decision` |
...
```mermaid
flowchart LR
    A[crawl_intraday_data] --> B[dbt_run] --> C[ml_predict] --> D[fp_scan_signals]
```
